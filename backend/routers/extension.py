...
def process_batch_background(payloads: List[schemas.ExtensionPayload], settings: schemas.Settings, user_id: int):
    ...
    for i in range(0, len(payloads), chunk_size):
        ...
        ai_results = batch_extract_job_details(jobs_for_ai, api_key, model_name, user_id)
        ...
        for j, payload in enumerate(chunk):
            try:
                ...
                crud.update_job_status(db, user_id, job.id, job_update)
                db.refresh(job)
...
def parse_title_endpoint(page_title: str, db: Session = Depends(get_db), current_user: models.User = Depends(auth.get_current_user)):
    ...
    api_key = settings.gemini_api_key if settings else None
    model_name = settings.gemini_model if settings else None
    parsed = parse_job_page_title(page_title, api_key, model_name, current_user.id)
    return parsed
...
def save_from_extension(payload: schemas.ExtensionPayload, db: Session = Depends(get_db), current_user: models.User = Depends(auth.get_current_user)):
    ...
    clean_desc = sanitize_job_description(payload.description, api_key, current_user.id)

    company = payload.company.strip() if payload.company else ""
    title = payload.title.strip() if payload.title else ""

    if not company or company == "Unknown Company":
        parsed = parse_job_page_title(payload.page_title, api_key, model_name, current_user.id)
        ...
        if (company == "Unknown Company" or title == "LinkedIn" or title == "Search") and payload.description:
            parsed_desc = extract_job_details_from_description(payload.description, api_key, model_name, current_user.id)
            if parsed_desc:
    ...
    job_update = schemas.JobUpdate(**update_data)
    crud.update_job_status(db, current_user.id, job.id, job_update)
    db.refresh(job)
    return job
