> **Vendored from [career-ops](https://github.com/career-ops-hq/career-ops)
> (MIT), unmodified.** The module contract, mandatory guards, and test
> requirements below apply exactly as written to `backend/universal/providers/`
> in this repo. A few things don't transfer directly:
> - Links to `../ARCHITECTURE.md`, `../CONTRIBUTING.md`,
>   `../docs/SUPPORTED_JOB_BOARDS.md`, `templates/portals.example.yml`, and
>   the `.github/ISSUE_TEMPLATE/source-proposal.yml` flow point at
>   career-ops's own repo, not this one — this repo has no equivalent files.
...
>   only if you're upstreaming a new provider *to career-ops itself* (see
>   `docs/agents/DEVELOPER_GUIDE.md` §1.B step 5) — not for adding one here.
>
> Everything else — the contract, the mandatory SSRF/pagination/retry guards,
> the reference-module table — is the actual authority for any provider added
> to `backend/universal/providers/`, vendored or new.
