...
venv/bin/python3 -m uvicorn backend.main:app --reload --reload-dir backend --reload-exclude "dump" --reload-exclude "*.log" --reload-exclude "*.db*" --port 8000 2>&1 | tee ".data/dump/run_$TS.log" &
BACKEND_PID=$!
