...
Run the `/security-review` skill before merging any change touching `backend/auth.py`, `backend/crowdsourcing.py`, `backend/crypto.py`, or the SSO routes — an earlier version of the SSO endpoint minted a local session for any Google/GitHub account holder with no allowlist (see `docs/agents/DEVELOPER_GUIDE.md` §5), so this boundary has broken before and is worth re-checking on every touch.
