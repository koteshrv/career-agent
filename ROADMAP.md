...
- [ ] **Community-Powered Job Pool (Crowdsourcing)**: Implement a distributed discovery model where the Chrome Extension explicitly extracts and sanitizes job listings to a central API. In exchange, users get instant access to a massive, community-built database of jobs that circumvents LinkedIn/Naukri scraping blocks. Requires SSO authentication for spam prevention and public anonymization to protect user identities.
