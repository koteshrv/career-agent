...
- [x] **Telegram Health Alerts** — `backend/notifications.py` calls the Telegram Bot API, triggered from `health_manager.update_health()` when a provider transitions to `BLOCKED`/`BROKEN` (or every 5th consecutive failure), plus scrape-run-crash and silent-zero-jobs alerts. Was actually wired up before but silently broken in three ways: it read the Fernet-encrypted bot token column directly instead of decrypting it (every alert 404'd), ignored the `telegram_alerts_enabled` toggle, and crashed with `TypeError` on a provider's first-ever recorded failure. All three fixed, plus Markdown-special-character escaping so an error message containing `_`/`*`/`` ` `` doesn't get the whole alert rejected by Telegram.
...
- [ ] **Multi-Dimensional Scoring Schema**: Move beyond the single 0-100 `match_score` to a structured categorical schema (grading Tech Stack, Experience Level, Domain Knowledge, Culture Fit from A-F) to provide a nuanced evaluation of job fit.
- [ ] **ATS Parseability Audit**: After LaTeX compilation, extract the text using `PyPDF2` and run it through a lightweight LLM pass to verify that critical details (Name, Email, Top Skills) are easily parseable by standard ATS systems.
- [ ] **Interview Prep (STAR Method)**: Leverage the ChromaDB RAG engine to automatically generate behavioral interview answers tailored to the job description using the STAR methodology.

## 4. Architectural & Codebase Health
- [x] **Scraper module split** — `backend/scraper_core.py` was a 1,470-line file mixing pure link-filtering logic, five per-vendor scrapers, and the Playwright engine. Split into `backend/sources/{common,greenhouse,lever,api_post,tech_mahindra,zwayam,playwright_engine}.py`, with `scraper_core.py` now a slim orchestrator that re-exports the public API for backward compatibility.
...
- [x] **Database Concurrency (SQLite WAL)** — Enabled `PRAGMA journal_mode=WAL;` and `synchronous=NORMAL` via SQLAlchemy connection events to allow simultaneous reads and writes from the API, APScheduler, and Playwright scrapers without hitting database locks.
...
- [x] **Automated Test Suite** — `pytest` suite in `tests/` covers scraper link filtering, job status transitions, settings encryption, and target health rollup. Runs in CI (`docker-publish.yml`) and blocks the Docker image build on failure.
