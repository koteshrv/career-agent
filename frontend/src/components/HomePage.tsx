  ...
  return (
    <div className="flex flex-col gap-8 max-w-5xl">
      ...
      <div className="grid grid-cols-3 gap-4">
        <Link to="/app/applications" className="bg-card border border-border rounded-lg p-4 hover:border-ring/40 transition-colors">
          <div className="flex items-center gap-2 text-xs text-muted-foreground mb-2">
            <span className="w-1.5 h-1.5 rounded-full bg-status-new" /> New matches
          </div>
          <p className="text-2xl font-semibold text-foreground">{newJobs.length}</p>
        </Link>
        <Link to="/app/applications" className="bg-card border border-border rounded-lg p-4 hover:border-ring/40 transition-colors">
          <div className="flex items-center gap-2 text-xs text-muted-foreground mb-2">
            <span className="w-1.5 h-1.5 rounded-full bg-status-applied" /> Applied
          </div>
          <p className="text-2xl font-semibold text-foreground">{applied.length}</p>
        </Link>
        <Link to="/app/applications" className="bg-card border border-border rounded-lg p-4 hover:border-ring/40 transition-colors">
          <div className="flex items-center gap-2 text-xs text-muted-foreground mb-2">
            <span className="w-1.5 h-1.5 rounded-full bg-status-interviewing" /> Interviewing
          </div>
          <p className="text-2xl font-semibold text-foreground">{interviewing.length}</p>
        </Link>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="bg-card border border-border rounded-lg overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-border">
            ...
            <Link to="/app/applications" className="text-xs text-primary hover:underline flex items-center gap-1">
        ...
        <div className="bg-card border border-border rounded-lg overflow-hidden">
          ...
          {interviewing.length === 0 ? (
            <p className="text-sm text-muted-foreground px-5 py-6">No interviews in progress.</p>
          ) : (
            <div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="bg-card border border-border rounded-lg p-5 flex flex-col gap-3">
          ...
          <button
            onClick={handleSync}
            disabled={isSyncing}
            className="flex items-center justify-between px-4 py-3 rounded-md bg-secondary hover:bg-accent border border-border transition-colors text-left disabled:opacity-50"
          >
          ...
          <Link to="/app/quick-generate" className="flex items-center justify-between px-4 py-3 rounded-md bg-secondary hover:bg-accent border border-border transition-colors">
            <span className="flex items-center gap-2 text-sm font-medium text-foreground">
              <Zap className="w-4 h-4" /> Quick Generate materials
            </span>
            <ArrowRight className="w-4 h-4 text-muted-foreground" />
          </Link>
          <Link to="/app/applications" className="flex items-center justify-between px-4 py-3 rounded-md bg-secondary hover:bg-accent border border-border transition-colors">
            <span className="flex items-center gap-2 text-sm font-medium text-foreground">
