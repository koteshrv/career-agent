...
from backend.scraper_core import load_targets, process_universal_api
...
async def test_all_targets(filter_companies: list = None):
    ...
    for target in targets:
        ...
        logger.info(f"Testing [{company}] via {target.get('provider') or 'auto-detect'}...")
        process_universal_api(db, DIAGNOSTIC_USER_ID, target, keywords, locations, new_jobs, company_logs)
...
"""
    ...
    asyncio.run(test_all_targets(filter_companies=args.companies))
